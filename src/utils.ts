...
export const parseCustomEmojis = (
  ...
  customEmojiData.forEach((item) => {
    const emojiIcon = item.match(
      /[\u{1F300}-\u{1F6FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F900}-\u{1F9FF}\u{1F1E0}-\u{1F1FF}]/gu,
    );
    const typeName = item.replace(
      /[\u{1F300}-\u{1F6FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F900}-\u{1F9FF}\u{1F1E0}-\u{1F1FF}]/gu,
      "",
    );
...
export const handleBranchData = async (options: ActionOptions) => {
  ...
  try {
    ...
    const ghPagesData = branchData.data.find(
      (item: any) => item.name === ghPagesBranch,
    );
    startGroup(`\x1b[34mGet Branch \x1b[0m`);
    info(`Branch Data: ${JSON.stringify(branchData.data, null, 2)}`);
    if (ghPagesData) {
      info(
        `ghPages Data: ${ghPagesBranch}, ${ghPagesData.commit.sha}, ${JSON.stringify(ghPagesData, null, 2)}`,
      );
...
export const fetchCommits = async (options: ActionOptions) => {
  ...
  if (myPath) {
    ...
    if (commitsData && commitsData.status !== 200) {
      setFailed(
        `There are no releases on ${options.owner}/${options.repo}. Tags are not releases. (status=${commitsData.status}) ${(commitsData.data as any).message || ""}`,
      );
    } else {
...
export const processCommits = (resultData: any[], options: ActionOptions) => {
  const { order, owner, repo, originalMarkdown, regExp, filterAuthor, types } =
    options;
  ...
  resultData.forEach((data) => {
    ...
    startGroup(
      `Commit: \x1b[34m${message}\x1b[0m \x1b[34m${(data.commit.author || {}).name}(${author.login})\x1b[0m ${data.sha}`,
    );
    info(`${JSON.stringify(data, null, 2)}`);
    endGroup();
    commitLog.push(
      formatStringCommit(message, `${owner}/${repo}`, {
        originalMarkdown,
        regExp,
...
export function formatStringCommit(
  commit = "",
  repoName = "",
  {
    regExp,
...
export function getRegExp(str = "", commit = "") {
  return new RegExp(`^(${str}\s+[\s|(|:])|(${str}[(|:])`).test(
    commit.trim().toLocaleLowerCase(),
  );
}
...
export function getCommitLog(log: string[], options = {} as Options) {
  ...
  log = log
    .map((commit) => {
      (Object.keys(types || {}) as Array<keyof typeof defaultTypes>).forEach(
        (name) => {
  ...
  if (template && typeof template === "string") {
    changelogContent = template.replace(
      /\{\{(.*?)\}\}/g,
      (string, replaceValue) => {
        const [typeString = "", emptyString] = (replaceValue || "").split("||");
        const arr = typeString
          .replace(/\s/g, "")
          .split(",")
          .map((name: keyof typeof types) => category[name] || [])
          .flat()
          .filter(Boolean);
