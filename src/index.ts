...
async function run() {
  try {
    ...
    info(
      `Ref: baseRef(\x1b[32m${options.baseRef}\x1b[0m), options.headRef(\x1b[32m${headRef}\x1b[0m), tagRef(\x1b[32m${options.tagRef}\x1b[0m)`,
    );
    ...
    if ((options.baseRef || "").replace(/^[vV]/, "") === headRef) {
      ...
      info(
        `Done: options.baseRef(\x1b[33m${options.baseRef}\x1b[0m) === headRef(\x1b[32m${headRef}\x1b[0m)`,
      );
      return;
    }
    ...
    if (regexp.test(headRef) && regexp.test(options.baseRef)) {
      ...
      const { changelog, changelogContent } = getCommitLog(commitLog, {
        types,
        showEmoji,
        removeType,
        template,
      ...
      setOutput(
        "compareurl",
        `https://github.com/${owner}/${repo}/compare/${options.baseRef}...${tagRef || headRef}`,
      );
      setOutput(
        "version",
        getVersion(tagRef || headRef || "").replace(/^v/, ""),
      );
    } else {
      setFailed(
        "Branch names must contain only numbers, strings, underscores, periods, forward slashes and dashes. (A-z 0-9 _ - / .)",
      );
    }
  } catch (error) {
    info(`path: ${error}`);
